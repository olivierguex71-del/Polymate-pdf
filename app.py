...
def radar_svg(dimensions, accent="#8FC819", accent2="#68B5FF"):
    ...
    for i, d in enumerate(dimensions):
        ...
        if lx < cx - 5:
            ...
            anchor = "start"
        label = d.get("title", "").split(" ")[0] if d.get("title") else "—"
        parts.append(f'<text x="{lx:.1f}" y="{ly:.1f}" font-size="11" fill="#66758d" text-anchor="{anchor}">{label}</text>')
...
def normalize_dimensions(dimensions):
    ...
    for d in dimensions or []:
        ...
        if not score_label:
            if d.get("score") is None and score_value == 0:
                score_label = "—"
            else:
                score_label = f"{int(score_value)}%"
...
def build_pdf(payload: dict) -> bytes:
    ...
    report_mode = meta.get("reportMode", "client")
    theme_name = meta.get("theme") or payload.get("theme", "cabinet")
    ...
    stability = scores.get("stability", payload.get("stability"))

    summary_html = payload.get("summary_html") or payload.get("summary") or ""
    if not summary_html:
        summary_html = "Aucune synthèse transmise."

    if not recommendation:
        recommendation = {
            "name": "Lecture partielle",
            "text": "Le diagnostic ne contient pas encore assez d’éléments pour formuler une recommandation plus précise.",
            "tags": ["À compléter"],
        }

    if not trajectory:
        if recommendation.get("name") == "Executive":
            trajectory = [
                {"title": "Séquence 1", "text": "Reprise en main immédiate."},
                {"title": "Séquence 2", "text": "Clarification et sécurisation."},
                {"title": "Séquence 3", "text": "Bascule vers Structure ou Core."},
            ]
        elif recommendation.get("name") == "Structure":
            trajectory = [
                {"title": "Séquence 1", "text": "Stabiliser l’existant."},
                {"title": "Séquence 2", "text": "Documenter et simplifier."},
                {"title": "Séquence 3", "text": "Passer ensuite à un pilotage Core."},
            ]
        elif recommendation.get("name") == "Core":
            trajectory = [
                {"title": "Séquence 1", "text": "Mettre un cockpit simple en place."},
                {"title": "Séquence 2", "text": "Instaurer un rythme de pilotage."},
                {"title": "Séquence 3", "text": "Conserver l’entretien global."},
            ]
        else:
            trajectory = [
                {"title": "Séquence 1", "text": "Compléter ou cibler le besoin."},
                {"title": "Séquence 2", "text": "Mesurer l’effet concret."},
                {"title": "Séquence 3", "text": "Élargir si nécessaire."},
            ]

    client_language = payload.get("client_language", []) or []
    if not client_language:
        if recommendation.get("name") == "Executive":
            client_language = [{"title": "Formulation possible", "text": "Votre entreprise a d’abord besoin d’une reprise en main claire et rapide."}]
        elif recommendation.get("name") == "Structure":
            client_language = [{"title": "Formulation possible", "text": "L’activité tient, mais l’organisation interne reste trop artisanale. Il faut consolider les fondations avant d’entretenir."}]
        elif recommendation.get("name") == "Core":
            client_language = [{"title": "Formulation possible", "text": "L’entreprise fonctionne, mais vous avez besoin d’un vrai pilotage transversal pour arbitrer sereinement."}]
        else:
            client_language = [{"title": "Formulation possible", "text": "Le besoin peut être traité de façon ciblée, sans déployer immédiatement une structure lourde."}]

    if report_mode == "client":
        # version client plus sobre si besoin, mais on garde le rapport complet
        pass

    rendered = PDF_TEMPLATE.render(
        theme=theme,
        meta={
            "client": meta.get("client", "Nom du client"),
            "date": meta.get("date", ""),
            "reference": meta.get("reference", "POLY-DIAG"),
            "owner": meta.get("owner", ""),
        },
        recommendation={
            "name": recommendation.get("name", "Lecture partielle"),
            "text": recommendation.get("text", ""),
            "tags": recommendation.get("tags", []),
        },
        scores={
            "maturity": int(scores.get("maturity", 0)),
            "urgency": int(scores.get("urgency", 0)),
            "completion": int(scores.get("completion", 0)),
        },
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, code=200, ctype="text/plain; charset=utf-8", data=b""):
    ...
    def do_GET(self):
        ...
        if path == "/":
            self._send(200, "text/plain; charset=utf-8", b"Polymate PDF server running")
            return
    ...
    def do_POST(self):
        ...
        try:
            ...
            filename = (payload.get("meta", {}) or {}).get("reference", "POLY-DIAG").replace(" ", "_")

            self.send_response(200)
            self._set_cors_headers()
            self.send_header("Content-Type", "application/pdf")
            self.send_header("Content-Disposition", f'attachment; filename="{filename}_polymate.pdf"')
...
if __name__ == "__main__":
    ...
    print(f"Polymate PDF server running on http://{host}:{port}")
